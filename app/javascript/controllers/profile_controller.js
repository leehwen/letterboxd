...
export default class extends Controller {
  ...
  tracker(e) {
    ...
    this.iconTargets.forEach((t1) => {
      if (e.target.dataset.userId == e.target.dataset.filmUser && e.target.dataset.icon == "watch" && e.target.dataset.category == "watchlist" && e.target.dataset.status == "inactive" && e.target.dataset.icon === t1.dataset.icon) {
        t1.classList.toggle("d-none")
        setTimeout(() => {this.filmTarget.classList.add("d-none")}, 800)
      } else if (e.target.dataset.icon == t1.dataset.icon) {
        t1.classList.toggle("d-none")
      }
    })
  }
